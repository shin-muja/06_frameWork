...
const searchAddress = document.querySelector("#searchAddress"); // 회원 정보 수정하기 버튼
...
if(updateInfo != null) {
    ...
    updateInfo.addEventListener("submit", async e => {
        ...
        if(currentNickname !== memberNickname.value) {
            ...
            // await : 비동기 작업의 결과를 기다릴 때 사용 -> !!! 반드시 async 함수 내에서만 사용 가능
            //       -> "이 작업이 끝날때까지 기다려주세요"
            ...
            if(count == 1) {
                ...
                // e.preventDefault(); 이벤트는 비동기의 응답과 상관없이 진행됨
                return;
            }
...
function execDaumPostcode() {
    new daum.Postcode({
        oncomplete: function(data) {
            ...
            document.getElementById("detailAddress").value = ""; // 기존 작성된 값 초기화
            // 커서를 상세주소 필드로 이동한다.
