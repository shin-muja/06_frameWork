...
public class EditBoardServiceImpl implements EditBoardService {
	...
	public int boardInsert(Board inputBoard, List<MultipartFile> images) throws Exception {
		...
		// 삽입 성공 시
		// 삽입된 게시글의 번호를 변수로 저장
		// -> mapper.xml에서 <selectKey> 태그로 이용해서 생성된
		...
		// 다중 ISNERT 성공 확인 (uploadList에 저장된 값이 모두 정상 삽입 되었는가)
	...
	public int boardUpdate(Board inputBoard, List<MultipartFile> images, String deleteOrderList)
		...
		// 2. 기존에 O -> 삭제된 이미지(boardOrderList)가 있는 경우
		...
		for(int i = 0 ; i < images.size(); i++ ) {
			...
			if( !images.get(i).isEmpty() ) {
				...
				if( result == 0 ) {
					// 수정 실패 == 기존 해당순서(IMG_ORDER)에 이미지가 없엇음
					// -> 삽입 수행
