...
public class EmailServiceImpl implements EmailService{
	...
	private final SpringTemplateEngine templateEngine;  // SpringTemplateEngine : 타임리프(템플릿 엔진)를 이용해서 html 코드 -> java 코드변환
	...
	public String sendEmail(String htmlName, String email) {
		...
		try {
			...
			//          기본값은 false로 false 설정 시 태그들까지 같이 보냄
			...
			e.printStackTrace();
			return null; // Exception 발생 == 메일 발송 실패 시
		}
	...
	// 인증키와 이메일을 DB에 젖아하는 메서드
	@Transactional(rollbackFor = Exception.class) // 메서드 레벨에서도 이용 가능(해당 메서드에서만 트랜젝션 커밋/롤백)
	...
	// 주로 DB에서 PK, 고유한 식별자를 생성해야 할 때 사용
