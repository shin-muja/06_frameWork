...
/*
  fetch() API
  비동기 요청을 수행하는 최신 Javascript AIP중 하나임

  - Promise 는 비동기 작업의결과를 처리하는 방법 중 하나
  -> 어떤 결과가 올지는 모르지만 반드시 결과를 보내주겠다라는 약속
  -> 비동기 작업이 맞이할 완료 또는 실패와 그 결가 값을 나타냄
  -> 비동기 작업이 완료 되었을 때 실행할 콜백 함수를 지정하고,
     해당 작업의 성공 또는 실패 여부를 처리할 수 있도록 함

  Promise 객체는 세 가지 상태를 가짐
  - Pending (대기 중) : 비동기 작업이 완료되지 않은 상태
  - Fuㅣfilled (이행됨) : 비동기 작업이 성공적으로 완료된 상태
  - Rejected (거부됨) : 비동기 작업이 실패한 상태

  - 콜백 함수 (callbach function)는 특정 작업이나 이벤트가 완료된 후 호출되는 함수
...
function getTotalCount() { // 함수 정의
  ...
  fetch("/ajax/totalCount") // 서버로 비동기 요청 (Default Get 요청)
  // 첫 번째 then (응답을 처리하는 역할)
  .then( response => {  // fetch가 서버에서 응답을 받으면,
                        // 이 응답(response)을 텍스트 형식으로 변환하는 콜백함수
    // 매개변수 response : 비동기 요청에 대한 응답이 답긴 객체

    // response.text() : 응답 데이터를 문자열/숫자 형태로
    //                   변환한 결과를 가지는 Promise 객체반환
...
function getCompleteCount() {
  ...
  // 두 번째 then() 의 result :
  // - 첫 번째 then에서 알맞게 데이터(test(), json())가 변환된 응답 데이터

  // response.text() : 응답 데이터를 텍스트 형태로 변환
  fetch("/ajax/completeCount")
  .then( res => res.text()) // == then( (res) => {return response.text() })
  .then( result => {
    // #completeCount 요소에 내용으로 reuslt 값 출력
...
addBtn.addEventListener("click", () => {
  ...
  // - 전달 데이터 : todoTilte.value, todoContent.value
  // JSON
  // { "name" : "홍길동",
  //   "age" : 20,
...
const selectTodoList = () => {
  ...
  .then( todoList => {
    ...
    // - JS Object 탕비을 string 형태의 JSON 데이터로 변환

    // console.log(todoList); // JS 객체 형태로 반환
    // ------------------------------------------------
    ...
    for( let todo of todoList) { // js에서 향상된 for문

      // tr 태그 생성
      const tr = document.createElement("tr");

      // JS 객체에 존재한 key 모음 배열
...
deleteBtn.addEventListener("click", () => {
  ...
  fetch("/ajax/delete", {
    ...
    // -> 본래는 이렇게 명시하는게 옮음(엄격한 환경에서는 이렇게 작성해야함)
  })
  .then( resp => resp.text() )
  .then(result => {
...
updateView.addEventListener("click", () => {
  ...
  // textarea 안에서는 <br>태그가 아닌 \n이 개행문자로 인식한다
  // 그렇기에 HTML화면에서 인식되는 <br>태그를 \n으로 전환 해줘야한다
...
// 수정 레이어에서 취소버튼(updateCancel) 클릭 시
...
updateBtn.addEventListener("click", e => {
  ...
  .then(result => {
    ...
    if( result > 0 ) {
      ...
      popupTodoContent.innerText = updateContent.value.replaceAll("\n", "<br>");

      updateTitle.value = ""; // 남은 제목 제거
      updateContent.value = ""; // 남은 내용 제거
